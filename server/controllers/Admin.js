...
class RequestController {
  ...
  */
  ...
  static getUsers(req, res) {
    const query = {
      text: 'SELECT id, username, email, firstname, lastname, user_role, created_at, updated_at FROM userlist WHERE userlist.id != $1 ORDER BY created_at DESC;',
  ...
  static filterQuery(reqStatus) {
    ...
    switch (reqStatus) {
      ...
      case 'reject':
      case 'rejected':
        query = 'SELECT user_id, userlist.firstname, userlist.lastname, userlist.email, request_id, title, details, currentstatus, requests.created_at FROM requests INNER JOIN userlist ON requests.user_id = userlist.id WHERE requests.currentstatus = \'rejected\' ORDER BY requests.created_at DESC LIMIT 10 OFFSET $1;';
        break;
      default:
